...
class ScorePolygon(
    MinMaxScaler, NumericHelper, VisualizeHelper, ColorsCollection
):
    """
    ...
    def get_scorepolygon(
        self, origin, rad, main_polygon, scoredict, sub_circle_rad
    ):
    ...
    def visualize(
        ...
        for edge in scorepolygon.Edges:
            self.visualize_curve(
                edge.ToNurbsCurve(), toggle, color=color, thickness=5
            )
