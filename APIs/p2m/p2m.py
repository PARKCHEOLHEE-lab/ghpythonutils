...
class P2M:
    ...
    def get_wall_image_cleaned(self) -> np.ndarray:
        ...
        _, image_binary = cv2.threshold(
            image_gray, self.THRESHOLD_BINARY, self.MAXVAL, cv2.THRESH_BINARY
        )

        num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(
            image_binary, connectivity=4
        )
        ...
        cleaned_image = cv2.morphologyEx(
            image_unconnected_removed, cv2.MORPH_CLOSE, kernel
        )
    ...
    def get_wall_coordinates(self) -> List[List[Tuple[int]]]:
        ...
        contours, _ = cv2.findContours(
            self.wall_image_cleaned, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE
        )

        image_boundary_area = (self.wall_image_cleaned.shape[0] - 1) * (
            self.wall_image_cleaned.shape[1] - 1
        )
